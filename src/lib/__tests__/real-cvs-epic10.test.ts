// CV Pulse — Epic 10 real CV paywall tests
// Tests the full usage gate + paywall logic across all 20 real CVs.
//
// The gate logic is pure (no Supabase needed) — we simulate:
//   Score 1 (existingCount=0)  → always free
//   Score 2 (existingCount=1, used=0) → free re-score allowed
//   Score 3 (existingCount=2, used=1) → paywalled (free user)
//   Score 3 (existingCount=2, used=1, paid) → allowed (paid bypass)
//
...
function simulateScoreGate(
  ...
  if (isPaywalled(usage, 'rescore')) {
    return { decision: 'blocked', reason: `paywalled — ${usage.free_rescores_used} re-scores used, paid_status=${usage.paid_status}` }
  }
  return { decision: 'allowed', reason: `re-score ${usage.free_rescores_used + 1} — within limits or paid` }
}
...
interface CVJourneyResult {
  ...
  stage3_paywalled: 'allowed' | 'blocked'          // existingCount=2, used=1, free → should BLOCK
  stage3_rolePulsePaid: 'allowed' | 'blocked'      // existingCount=2, used=1, rolepulse_paid → should allow
  stage3_stripePaid: 'allowed' | 'blocked'         // existingCount=2, used=1, paid_stripe → should allow
...
describe('Epic 10 — Real CV × paywall gate journey', () => {
  ...
  for (const filename of ALL_CVS) {
    it(`Full journey: ${filename}`, async () => {
      ...
      // Stage 3a: Second re-score (existingCount=2, used=1, free) — should BLOCK
      const g3_free = simulateScoreGate(2, makeUsage({ free_rescores_used: 1, paid_status: 'free' }))

      // Stage 3b: Same but RolePulse paid — should ALLOW
      const g3_rp = simulateScoreGate(2, makeUsage({ free_rescores_used: 1, paid_status: 'rolepulse_paid' }))

      // Stage 3c: Same but Stripe paid — should ALLOW
      const g3_stripe = simulateScoreGate(2, makeUsage({ free_rescores_used: 1, paid_status: 'paid_stripe' }))
      ...
      const result: CVJourneyResult = {
        ...
        stage3_paywalled: g3_free.decision,
      ...
      console.log(
        ...
        `gate: score1=${g1.decision} rescore=${g2.decision} rescore2=${g3_free.decision} paid_bypass=${g3_rp.decision}\n` +
      ...
      // First re-score (free_rescores_used=0) ALWAYS allowed
      expect(result.stage2_freeRescore).toBe('allowed')

      // Second re-score (free_rescores_used=1, free user) ALWAYS blocked
      expect(result.stage3_paywalled).toBe('blocked')
  ...
  it('prints paywall gate summary', () => {
    ...
    const allBlockedFree   = processed.every((r) => r.stage3_paywalled === 'blocked')
    ...
    console.log('\n──────── PAYWALL GATE SUMMARY ────────')
    console.log(`CVs processed: ${processed.length} / ${ALL_CVS.length} (${skipped.length} skipped)`)
    console.log(`Avg score: ${avgScore}/100`)
    console.log('')
    console.log('Gate behaviour across all CVs:')
    console.log(`  Score 1 (first score):       ${allAllowedFirst ? '✅ ALL allowed' : '❌ UNEXPECTED BLOCKS'}`)
    console.log(`  Score 2 (first re-score):    ${allAllowedRescore ? '✅ ALL allowed' : '❌ UNEXPECTED BLOCKS'}`)
    console.log(`  Score 3 (free user limit):   ${allBlockedFree ? '✅ ALL blocked' : '❌ UNEXPECTED ALLOWS'}`)
    console.log(`  Score 3 (rolepulse_paid):    ${allBypassRolePulse ? '✅ ALL bypassed' : '❌ UNEXPECTED BLOCKS'}`)
    console.log(`  Score 3 (paid_stripe):       ${allBypassStripe ? '✅ ALL bypassed' : '❌ UNEXPECTED BLOCKS'}`)
    console.log('')
    console.log('JD check gate:')
    ...
    expect(allBlockedFree).toBe(true)
...
describe('Epic 10 — Usage gate boundary conditions', () => {

  it('Free user: exactly at the re-score limit (used=1) is blocked', () => {
    const usage = makeUsage({ free_rescores_used: 1, paid_status: 'free' })
    expect(simulateScoreGate(2, usage).decision).toBe('blocked')
  })

  it('Free user: one below the re-score limit (used=0) is allowed', () => {
    const usage = makeUsage({ free_rescores_used: 0, paid_status: 'free' })
  ...
  it('existingScoreCount=0 is ALWAYS first score, regardless of high usage_used count', () => {
    // Edge case: a user who had a previous CV and used up re-scores,
    // then uploads a new CV — their first score on the NEW CV should still be free.
